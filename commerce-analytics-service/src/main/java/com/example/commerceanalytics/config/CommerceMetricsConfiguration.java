package com.example.commerceanalytics.config;
...
public class CommerceMetricsConfiguration {
    ...
    public MeterRegistryCustomizer<MeterRegistry> metricsCommonTags(Environment environment) {
        return registry -> {
            ...
            registry.config()
                    .commonTags(
                        "application", "commerce-analytics-service",
                        "service", "commerce-analytics",
    ...
    public MeterRegistryCustomizer<MeterRegistry> meterFilters() {
        return registry -> {
            ...
            registry.config()
                    ...
                    // Accept only specific JVM metrics and our commerce metrics to reduce noise
                    .meterFilter(MeterFilter.accept(id -> 
                        ...
                        id.getName().startsWith("commerce.")));
        };
    }
}
